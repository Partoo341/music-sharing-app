...
const Multipads = () => {
    ...
    const fetchMultipads = async () => {
        try {
            const q = query(
                collection(db, 'uploads'),
                where('category', '==', 'multipads'),
            ...
            console.error('Error fetching multipads:', error);
            setFiles([]);
        } finally {
            setLoading(false);
        }
    ...
    if (loading) {
        return (
            <div className="category-container">
                <div className="loading">Loading multipads...</div>
            </div>
        );
    }
            ...
            <div className="files-grid">
                {filteredFiles.length === 0 ? (
                    <div className="no-files">
                        {searchTerm ? 'No multipads match your search.' : 'No multipads available yet.'}
                    </div>
                ) : (
