...
const AudioBeats = () => {
    ...
    const fetchAudioBeats = async () => {
        try {
            const q = query(
                collection(db, 'uploads'),
                where('category', '==', 'audio-beats'),
            ...
            console.error('Error fetching audio beats:', error);
            setFiles([]);
        } finally {
            setLoading(false);
        }
    ...
    if (loading) {
        return (
            <div className="category-container">
                <div className="loading">Loading audio beats...</div>
            </div>
        );
    }
    ...
    return (
        <div className="category-container">
            ...
            <div className="search-section">
                <input
                    type="text"
                    placeholder="Search audio beats..."
                    value={searchTerm}
            ...
            <div className="files-grid">
                {filteredFiles.length === 0 ? (
                    <div className="no-files">
                        {searchTerm ? 'No audio beats match your search.' : 'No audio beats available yet.'}
                    </div>
                ) : (
